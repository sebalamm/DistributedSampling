...
int main(int argn, char **argv) {
    ...
    for (ULONG i = 0; i < config.iterations; ++i) {
        ...
        std::cout << "sequential divide-and-conquer\n";
        std::cout << "warmup... ";
        ...
        std::cout << "parallel divide-and-conquer\n";
        std::cout << "warmup... ";
    ...
    for (ULONG i = 0; i < config.iterations; i++) results[i].second /= config.iterations;
    std::sort(results.begin(), results.end(), [](const std::pair<std::string, double> &left, const std::pair<std::string, double> &right) {
            return left.second < right.second; 
        });

    std::cout << "ranking on average speed over " << config.iterations << " iterations\n";
    std::cout << "==============================\n";
